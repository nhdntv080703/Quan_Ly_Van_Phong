...
public class UserService implements IUserService {
    ...
    @Autowired
    private StaffResponseDTOConverter staffResponseDTOConverter;
    ...
    @Override
    public List<StaffResponseDTO> findAllUser() {
        List<UserEntity> userEntities = userRepository.findAllUser();
        List<StaffResponseDTO> staffs = staffResponseDTOConverter.converToStaffResponseDTO(userEntities) ;
        return staffs;
    }
    @Override
    public List<StaffResponseDTO> getAllUsers() {
//        Page<UserEntity> users = null;
//        if (StringUtils.isNotBlank(searchValue)) {
//            users = userRepository.findByUserNameContainingIgnoreCaseOrFullNameContainingIgnoreCaseAndStatusNot(searchValue, searchValue, 0, pageable);
//        } else {
